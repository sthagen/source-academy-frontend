...
import { ADD_COMMENT_ASSESSMENT, ADD_COMMENT_SUBMISSION, REMOVE_COMMENT_ASSESSMENT, REMOVE_COMMENT_SUBMISSION, UPDATE_COMMENT_ASSESSMENT, UPDATE_COMMENT_SUBMISSION } from "./CommentTypes";
...
export const CommentsReducer: Reducer<SessionState> = (
    ...
    switch(action.type) {
        case ADD_COMMENT_SUBMISSION: {
            ...
            newGradings.set(submissionId, oldGrading.map((gradingQuestion, qId) => {
                if(qId === questionId) {
                    return {
                        ...gradingQuestion,
        ...
        case REMOVE_COMMENT_SUBMISSION: {
            ...
            const newCommentsAtLine: IComment[] = oldCommentsAtLine.filter(comment => comment.id !== commentId);
