...
function CommentCard(props: {
  comment: IComment;
  editComment: (IComment) => (any) => void;
  collapseComment: (IComment) => void;
  isUnsubmittedComment: (c: IComment) => boolean;
  updatePreviewCommentText: (comment: IComment) => (e: any) => void;
  cancelWithPrompt: (c: IComment) => (e: any) => void;
  confirmSubmit: (c: IComment) => (e: any) => void;
}) {
  const {
    comment,
    editComment,
...
export default function Comments(props: CommentsProps) {
  ...
  const setCollapseAll = React.useCallback(
    (status: boolean) => {
      if (status) {
  ...
  return (
    <div className="comments-container" style={commentsContainerStyles}>
      ...
      >
        {comments.length > 0 ? <Icon icon={isCollapsed ? 'small-plus' : 'small-minus'}></Icon> : ''}
      </div>
